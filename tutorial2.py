...
def build_model2(config: ConfigDict, vocab_src_len: int, vocab_tgt_len: int) -> Transformer2:
    model = build_transformer2(
        vocab_src_len,
        vocab_tgt_len,
        config["seq_len"],
...
def train_model2(config: ConfigDict) -> None:
    device = get_device()

    model_folder = get_model_folder(config)
    Path(model_folder).mkdir(parents=True, exist_ok=True)

    train_dataloader, _val_dataloader, tokenizer_src, tokenizer_tgt = get_ds2(config, model_folder)
    ...
    for epoch in range(initial_epoch, config["num_epochs"]):
        ...
        for batch_num, batch in enumerate(batch_iterator):
            ...
            loss = loss_fn(output.contiguous().view(-1, tokenizer_tgt.get_vocab_size()), tgt_data.contiguous().view(-1))
            ...
            if (batch_num > 0) and (batch_num % 100 == 0):
                ...
                kn_sentence_predicted = torch.argmax(output[0], dim=1)
...
def evaluate_model2(
    ...
    # JEB: Transformer2 does not (yet) expose a greedy_decode helper like Transformer1,
    # so this evaluation path is not wired up. The original body referenced an undefined
    # `model_out` (the greedy_decode call was commented out), which would have raised a
    # NameError if this function were ever called. It is preserved below for reference.
    raise NotImplementedError("evaluate_model2 requires a greedy_decode implementation for Transformer2")
...
def translate2(config: ConfigDict, sentence: str) -> str:
    device = get_device()
    ...
    sentence, _label, tokenizer_src, tokenizer_tgt = get_testing_ds2(config, model_folder, sentence)
    model = build_model2(config, tokenizer_src.get_vocab_size(), tokenizer_tgt.get_vocab_size()).to(device)
...
def debug_code_model2(config: ConfigDict, device: str) -> None:
    config["model"] = "model2"
    ...
    _train_dataloader, _val_dataloader, _tokenizer_src, _tokenizer_tgt = get_ds2(config, model_folder)

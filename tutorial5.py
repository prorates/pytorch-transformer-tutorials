...
def build_model5(config: ConfigDict, vocab_src_len: int, vocab_tgt_len: int) -> Transformer5:
...
def train_model5(config: ConfigDict) -> None:
    device = get_device()
    ...
    _model = build_model5(config, tokenizer_src.get_vocab_size(), tokenizer_tgt.get_vocab_size()).to(device)
    raise RuntimeError("Training for model5 not implemented")


def translate5(config: ConfigDict, sentence: str) -> None:
    device = get_device()
...
def debug_code_model5(config: ConfigDict, device: str) -> None:
    config["model"] = "model5"

...
- [ ] **model8 attention is unbatched (slow on MPS)** — `MultiHeadAttention` is a `ModuleList` of separate `Head` modules looped in Python, so each step is ~450 ms for a 10.8M-param model on MPS (a batched `scaled_dot_product_attention` impl would be ~10–20×). Memory is fine (5.2 GB peak); the cost is wall-clock (~38 min for 5000 iters). Consider a batched-QKV rewrite of `model8.py` if training time becomes a pain.

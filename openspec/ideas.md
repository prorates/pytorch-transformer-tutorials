...
- [ ] **Reconcile auto-generated config defaults with the committed `config.yaml`** — `get_config()` with no `-c`/`-m` auto-writes `<datasource>_<src>_<tgt>_<model>/config.yaml` from `get_default_config()` on first run (`config.py:44-50`), then reads it back as source of truth. But the in-code defaults describe a much bigger model than the committed `tinyshakespeare_en_en_model8/config.yaml`: defaults `d_model=256, N=6, h=8, d_ff=1024, batch_size=8, num_epochs=20, lr=1e-4` vs committed `d_model=64, N=4, h=4, d_ff=256, batch_size=16, num_epochs=5, lr=1e-3`. Footgun: deleting/regenerating the config silently swaps in a ~4× wider, 6-layer architecture. Options: (a) align `get_default_config()` to the small laptop-friendly committed values, (b) have training log which config it generated vs loaded and warn on mismatch, (c) document the intended defaults. Also note editing `get_default_config()` has no effect once a folder's `config.yaml` exists.

...
Train any of the eight implementations from one entrypoint and one config.

`train.py` reads a config and dispatches on `config["alt_model"]`, which is what makes the
implementations comparable in practice: same CLI, same config shape, same checkpoint layout,
so the only variable is the model. Every name in the dispatch table must be imported — a
commented-out import turns selecting a model into a `NameError` at the point of use.

...
Turn a trained checkpoint back into text, through the same dispatch the trainer used.

`translate.py` resolves config and device exactly as `train.py` does, then dispatches on
`config["alt_model"]`, so inference cannot drift from training. A missing checkpoint or
model folder is an error that names the path, because the alternative — a randomly
initialised model producing fluent nonsense — reads as a model quality problem.

...
Make one codebase run unchanged on CUDA, Apple Silicon, and CPU.

Backend selection happens in exactly one place (`config.get_device()`, CUDA → MPS → CPU) and
everything else asks it. This capability exists because the failure it prevents is quiet: a
hard-coded `.cuda()` is a no-op or a crash on MPS, and a backend-specific call like
`torch.cuda.empty_cache()` raises on a machine that has no CUDA at all.

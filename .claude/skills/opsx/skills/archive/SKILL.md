...
<!-- alemax-todowrite-fallback (GENERATED by plugins/opsx/build.py — do not edit) -->

## TodoWrite fallback

If the `TodoWrite` tool is not in your available tools (some Claude harness configurations don't expose it), edit the change's `tasks.md` checkboxes directly: set `[ ]` to `[x]` on completion. That file is the durable progress tracker either way. References to `TodoWrite` below should be read as "use TodoWrite if available; otherwise edit tasks.md."

See `meta/docs/HOW-TO-USE-CLAUDE.md` for the broader workflow context.

<!-- /alemax-todowrite-fallback -->
